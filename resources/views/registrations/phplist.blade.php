      ...
      <div class="col-md-12">
          <div class="card border-0 shadow-lg my-3">
              <div class="card-header bg-dark text-center d-flex justify-content-between align-items-center">
                  <h3 class="text-white mb-0">PHP Course</h3>
                  <div>
              ...
              <div class="card-body">
                  <div class="table-responsive">
                      <table id="registrations-table" class="table table-bordered table-hover mx-auto table-responsive-sm table-responsive-md table-responsive-lg">
                          <thead>
                              <tr>
                                  <th>ID</th>
                                  <th>Full Name</th>
                          ...
                          <tbody>
                              ...
                              @empty
                                  <tr>
                                      <td colspan="10" class="text-center">No confirmed registrations found</td>
                                  </tr>
                              @endforelse
                          </tbody>
                      </table>
...
<script>
    ...
    document.getElementById('download-csv').addEventListener('click', function () {
        ...
        link.setAttribute('download', 'PHP-course.csv');
    ...
    document.getElementById('download-pdf').addEventListener('click', function () {
        ...
        const headers = Array.from(header).filter((th, index) => index !== 8 && index !== 9).map(th => th.innerText);

        const data = rows.slice(1).map(row => {
            const cols = row.querySelectorAll('td');
            return Array.from(cols).filter((td, index) => index !== 8 && index !== 9).map(td => td.innerText);
        });
