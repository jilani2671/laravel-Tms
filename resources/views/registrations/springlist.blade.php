      ...
      <div class="col-md-12">
          <div class="card border-0 shadow-lg my-3">
              <div class="card-header bg-dark text-center d-flex justify-content-between align-items-center">
                  <h3 class="text-white mb-0">spring-boot Course</h3>
                  <div>
...
<script>
    // Calculate and add the Pending Fees column
    document.addEventListener('DOMContentLoaded', function() {
        const rows = document.querySelectorAll('#registrations-table tbody tr');
        rows.forEach(row => {
            const totalFees = parseFloat(row.cells[7].innerText);
            const installment1 = parseFloat(row.cells[5].innerText);
            const installment2 = parseFloat(row.cells[6].innerText);
    ...
    document.getElementById('download-csv').addEventListener('click', function () {
        ...
        link.setAttribute('download', 'Spring-course.csv');
